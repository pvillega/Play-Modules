...
// place any jQuery/helper plugins in here, instead of separate, slower script files. Add them minified!
// Rich text editor -  Pagedown http://code.google.com/p/pagedown/  minimized via http://fmarcia.info/jsmin/test.html
// Converter
...
//  Pjax v 9 May 2012 https://github.com/defunkt/jquery-pjax - minimized via http://fmarcia.info/jsmin/test.html
(function($){$.fn.pjax=function(container,options){return this.live('click.pjax',function(event){handleClick(event,container,options)})}
...
return
var defaults={url:link.href,container:$(link).attr('data-pjax'),target:link,clickedElement:$(link),fragment:null}
...
var timeoutTimer
options.beforeSend=function(xhr,settings){if(settings.timeout>0){timeoutTimer=setTimeout(function(){if(fire('pjax:timeout',[xhr,options]))
...
if(!container.contents){window.location=container.url
return}
...
if(xhr.readyState>0){$(document).trigger('pjax',[xhr,options])
if(options.push&&!options.replace){containerCache.push(pjax.state.id,context.clone(true,true).contents())
...
function findAll(elems,selector){var results=$()
elems.each(function(){if($(this).is(selector))
...
if(options.fragment){var $fragment=findAll($data,options.fragment).first()
if($fragment.length){obj.contents=$fragment.contents()
if(!obj.title)
obj.title=$fragment.attr('title')||$fragment.data('title')}}else if(!/<html/i.test(data)){obj.contents=$data}
if(obj.contents){obj.contents=obj.contents.not('title')
obj.contents.find('title').remove()}
if(obj.title)obj.title=$.trim(obj.title)
return obj}
pjax.reload=function(container,options){var defaults={url:window.location.href,push:false,replace:true,scrollTo:false}
return $.pjax($.extend(defaults,optionsFor(container,options)))}
pjax.defaults={timeout:650,push:true,replace:false,type:'GET',dataType:'html',scrollTo:0,maxCacheLength:20}
function Cache(){this.mapping={}
this.forwardStack=[]
this.backStack=[]}
Cache.prototype.push=function(id,value){this.mapping[id]=value
...
Cache.prototype.get=function(id){return this.mapping[id]}
Cache.prototype.forward=function(id,value){this.mapping[id]=value
this.backStack.push(id)
if(id=this.forwardStack.pop())
delete this.mapping[id]}
Cache.prototype.back=function(id,value){this.mapping[id]=value
...
if(container.length){var contents=containerCache.get(state.id)
...
if(contents){$(document).trigger('pjax',[null,options])
...
container.trigger('end.pjax',[null,options])}else{$.pjax(options)}
container[0].offsetHeight}else{window.location=location.href}}})
